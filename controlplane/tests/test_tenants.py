...
def test_no_config_returns_empty(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.delenv("PRIVACYHOOK_CONTROLPLANE_TENANTS_PATH", raising=False)
...
def test_legacy_single_tenant_fallback(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.delenv("PRIVACYHOOK_CONTROLPLANE_TENANTS_PATH", raising=False)
    monkeypatch.setenv("PRIVACYHOOK_CONTROLPLANE_TOKEN", "dev-token")
    monkeypatch.setenv("PRIVACYHOOK_CONTROLPLANE_POLICY_PATH", str(policy))
...
def test_multi_tenant_missing_file_rejected(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("PRIVACYHOOK_CONTROLPLANE_TENANTS_PATH", str(tmp_path / "nope.yaml"))
    with pytest.raises(TenantConfigError):
        load_tenants()
